...
function addNotificationButton() {
  ...
  notifyBtn.id = 'notification-btn';  // This ID must match the CSS selector
  
  // Set initial state based on stored preference and update text
  if (notificationState.enabled && Notification.permission === 'granted') {
    notifyBtn.textContent = 'Alerts: ON';
    notifyBtn.classList.add('active');  // Add the active class to change color
  } else {
    notifyBtn.textContent = Notification.permission === 'granted' ? 'Alerts: OFF' : 'Enable Alerts';
  }
  ...
  console.log('Button created, enabled:', notificationState.enabled, 'permission:', Notification.permission);
}
...
function updateNotificationButton(button) {
  ...
  console.log('Button updated, enabled:', notificationState.enabled, 'classes:', button.className);
}
